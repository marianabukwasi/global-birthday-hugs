...
const Dashboard = () => {
  ...
  const quickActions = [
    ...
    { icon: Gift, label: "Glimmer Draw", desc: "Check your birthday prizes", to: "/spins", color: "bg-primary/20 text-primary" },
    { icon: Calendar, label: "My Capsules", desc: "View past birthday capsules", to: "/timeline", color: "bg-celebration-purple/20 text-celebration-purple" },
      ...
      <main className="container mx-auto px-4 pt-24 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <h1 className="font-display text-4xl font-bold text-foreground mb-2">
            Welcome, <span className="text-gradient-gold">{profile?.preferred_name || profile?.full_name || "Celebrant"}</span>
          </h1>
        ...
        {!hasBirthdayPage && (
          ...
          >
            <div className="rounded-2xl bg-background/95 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <Button onClick={handleCreateBirthdayPage} className="bg-gradient-gold text-primary-foreground border-0 hover:opacity-90 whitespace-nowrap">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: "Wishes Sent", value: stats.total_wishes.toLocaleString(), icon: Cake, color: "text-primary" },
